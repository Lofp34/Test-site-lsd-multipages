...
const category = bookCategories.find(cat => cat.slug === 'methodes-process');
...
export default function MethodesProcessPage() {
  ...
  return (
    <>
      <Head>
        <title>Méthodes & Processus de vente | Meilleurs livres | LSD</title>
        <meta name="description" content="Les meilleurs livres pour structurer votre démarche commerciale. Résumés, avis, conseils terrain sur SPIN, Challenger, Solution Selling, etc." />
      </Head>
      <main className="bg-gradient-to-br from-blue-ink via-mint-green/10 to-primary-bg min-h-screen pt-24 pb-16">
        ...
        <section className="max-w-4xl mx-auto text-center mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 drop-shadow-lg">Méthodes & Processus de vente</h1>
          <p className="text-lg md:text-xl text-white/80 mb-6">Cadres éprouvés, de SPIN à Challenger. Les meilleures méthodes pour structurer votre démarche commerciale.</p>
        </section>
        ...
        <AnimatedSection delay={0.1}>
          <div className="max-w-3xl mx-auto bg-white/70 dark:bg-blue-ink/80 rounded-xl shadow-lg p-6 mb-10 border border-mint-green/20">
                  ...
                  {category.books.map(book => (
                    <tr key={book.slug}>
                      <td className="py-2 px-3 font-semibold">
                        <Link href={`/ressources/meilleurs-livres/methodes-process/${book.slug}`} className="text-mint-green underline hover:text-mint-green/80">
                          {book.title}
                        </Link>
                      </td>
                      ...
                      <td className="py-2 px-3">Vendeur, Manager</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="mt-4 text-right">
              <Link href="#quiz" className="text-mint-green underline hover:text-mint-green/80">👉 Quelle méthode pour votre profil ? (Quiz à venir)</Link>
            </div>
          </div>
        </AnimatedSection>
                ...
                <div key={idx} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
                  {row.map(book => (
                    <div key={book.slug} className="bg-white/80 dark:bg-blue-ink/80 rounded-2xl shadow-2xl p-6 flex flex-col items-center glassmorphism hover:scale-105 hover:shadow-3xl transition-all duration-300 border border-mint-green/30">
                      <div className="w-28 h-40 bg-gradient-to-br from-mint-green/30 to-blue-ink/10 rounded-lg mb-4 flex items-center justify-center text-4xl">🛠️</div>
                      ...
                      <Link href={`/ressources/meilleurs-livres/methodes-process/${book.slug}`} className="mt-auto inline-block bg-mint-green text-blue-ink font-semibold px-4 py-2 rounded-full shadow hover:bg-mint-green/80 transition">Résumé complet</Link>
                    </div>
                  ))}
